...
import numpy as np
import torch
from CaMeL.encoder import gaussian_encode
...
class ILCM(BaseLCM):
    def __init__(
        ...
        self.three_d_spiking_memory = three_d_spiking_memory
    def forward(
        self,
        x1,
        x2,
        ...
        **kwargs,
        ...
        three_d_spiking_outputs = {}
        if self.three_d_spiking_memory is not None:
            three_d_spiking_outputs = self.three_d_spiking_memory(e1_mean, e2_mean, intervention_posterior)
            intervention_posterior = self.three_d_spiking_memory.blended_posterior(
                intervention_posterior, three_d_spiking_outputs["three_d_spiking_intervention_posterior"]
            )
        ...
        if three_d_spiking_outputs:
            outputs.update(three_d_spiking_outputs)
        return loss, outputs
    ...
    def forward_pretrain(self, x1, x2, beta, full_likelihood=False, likelihood_reduction="sum"):
        ...
        if hasattr(self.scm, "three_d_regularizers"):
            outputs.update(self.scm.three_d_regularizers())
